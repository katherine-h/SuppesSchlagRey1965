...
In exploratory analyses, we will also plot the proportion of switches as a function of trial, similar to Figure 1A in (Suppes and Schlag-Rey 1965), and will determine whether the observed proportion of switches on negative reinforcement trials is significantly greater than 0.05.
...
compute_cis_by_trial <- function(reinforcement_type_trials) {
  ...
  for (i in (0:7)) {
    ...
    if (length(instances_this_trial) > 0) {
      ...
      out <- binom_test(x, n)
      cis_lower <- c(cis_lower, out$conf.int[1])
      cis_upper <- c(cis_upper, out$conf.int[2])
    }
  }
...
# Exploratory Analysis 2: Test whether switching on negative reinforcement trials is significantly above 0.05 (small noise probability)
...
<!--
To be written following data collection.
